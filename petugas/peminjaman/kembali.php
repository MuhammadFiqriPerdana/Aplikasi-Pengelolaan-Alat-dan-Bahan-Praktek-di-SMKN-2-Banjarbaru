<?php
    if(isset($_GET['kode'])){
        $jumlah = $_GET['jumlah'];
        $selSto = mysqli_query($koneksi, "SELECT * FROM tb_barang WHERE kode_barang='" . $_GET['kode_barang'] . "'");
    ...
    if($sql_ubah) {
        $upstok = mysqli_query($koneksi, "UPDATE tb_barang SET stok='$hasil' WHERE kode_barang='" .$_GET["kode_barang"]. "'");
    }
